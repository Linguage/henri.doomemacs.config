...
(setq org-capture-templates
      '(("d" "个人日记" entry (file+olp+datetree "~/Documents/EmacsNotes/Journal/diary.org")
        ...
        ("s" "学习日志" entry (file+olp+datetree (expand-file-name "studylog.org" org-directory))
         "* %U %? :journal:study:\n%i\n** 主题与工作\n\n** 要点笔记\n\n** 资源链接\n"
         :empty-lines 1)))

;; 设置 Org-mode 的 Agenda 文件 - 统一路径命名（基于 org-directory）
(setq org-agenda-files (list (expand-file-name "diary.org" org-directory)
                             (expand-file-name "worklog.org" org-directory)
                             (expand-file-name "studylog.org" org-directory)))
...
JOURNAL-TYPE 可以是 'diary'(个人日记), 'work'(工作日志) 或 'study'(学习日志)。"
  ...
  (let* ((journal-file (cond ((string= journal-type "work") (expand-file-name "worklog.org" org-directory))
                              ((string= journal-type "study") (expand-file-name "studylog.org" org-directory))
                              (t (expand-file-name "diary.org" org-directory))))
